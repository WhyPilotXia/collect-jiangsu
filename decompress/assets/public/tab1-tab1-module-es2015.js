...
AppConfig.devConfig = {
    ...
    url: {
        ...
        commonBusinessDataUrls: [
            { url: AppConfig_1.baseApi + '/business/datalist1', key: 'datalist1' },
            { url: AppConfig_1.baseApi + '/business/datalist2', key: 'datalist2' },
        ] // app自用的通用业务数据，缓存进localStorage，方便用于下拉选择，例如设备类型、检查类型、检查部位等
    },
    auth: {
        // 请求头信息，固定格式，不需要修改
        authHeaderOptions: { 'Content-type': 'application/json', 'authorization': '' },
        authPrefix: 'Bearer ',
        authServerClientId: 'demo-id',
        authServerPassword: 'demo-secret',
        ignoreList: [
            AppConfig_1.baseApi + '/register'
        ]
    },
    date: {
        dateFormat: 'yy/mm/dd',
...
AppConfig = AppConfig_1 = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([
...
let Tab1Page = class Tab1Page {
    constructor(router, http, platform, eleRef, alertModal, route, changeRef) {
        this.router = router;
        this.http = http;
    ...
    ngOnInit() {
        ...
        this.route.queryParams.subscribe(data => {
            if (data['boxBarcode'] !== undefined) {
                this.http.showToast('封管失败，' + data['boxBarcode'] + '箱子已满，请扫码新的箱码，该试管会自动封管并关联到新的箱子中。');
                this.isMove = true;
            }
            else {
                this.isMove = false;
    ...
    OpenBoxOperate(barcode) {
        ...
        this.http
            .get(`rcp/api/box/operate/scan`, condition, '扫描转运箱条码', _environments_environment__WEBPACK_IMPORTED_MODULE_10__["environment"].FailMessage.openBox)
            .subscribe(data => {
    ...
    confirmTube() {
        ...
        this.http
            .get(`rcp/api/tube/operate/confirmTube`, condition, '确认采集管', _environments_environment__WEBPACK_IMPORTED_MODULE_10__["environment"].FailMessage.confirmTube)
            .subscribe(data => {
            this.load['style']['display'] = 'none';
            if (data['isSuccess']) {
                this.router.navigate(['/BottleList']);
    ...
    getSampleByUser() {
        ...
        this.http
            .get(`rcp/api/box/operate/getCollectNumByUser`, condition, '根据采集员查询采集相关数量', '')
            .subscribe(data => {
            this.load['style']['display'] = 'none';
            if (data['isSuccess']) {
                this.sampleInfo = data['data'];
            }
        });
    }
    // 根据用户查管
    getTubeByUser() {
        ...
        this.http
            .get(_app_config__WEBPACK_IMPORTED_MODULE_7__["AppConfig"].baseApi + `rcp/api/tube/operate/getTubeByUser`, condition, '根据用户查管', '')
            .subscribe(data => {
            // 判断是否有管
            if (Object(codelyzer_util_isNotNullOrUndefined__WEBPACK_IMPORTED_MODULE_3__["isNotNullOrUndefined"])(data['data']['tubeId'])) {
                ...
                sessionStorage.setItem('tubeCapacity', data['data']['tubeCapacity']);
                //  跳转到扫描身份证界面
    ...
    // 根据箱编码查询箱信息
    getBoxInfoById(barcode) {
        this.load['style']['display'] = 'inline-block';
        const condition = {
            boxBarcode: barcode
        };
        this.http
            .get(`rcp/api/box/operate/getBoxInfoByBarcode`, condition, '根据箱编码查询箱信息', '查询箱信息失败')
            .subscribe(data => {
            this.load['style']['display'] = 'none';
            if (data['isSuccess']) {
                switch (data['data']['boxState']) {
                    case 'OpenBox':
                        ...
                        break;
                    case 'CloseBox':
                        this.showBoxInfoAlert(data['data']);
                        break;
                    case 'Send':
                        this.http.showToast('该转运箱已送检');
                        break;
                    case 'Incept':
                        this.http.showToast('该转运箱已接收');
                        break;
                    default:
                        this.showBoxInfoAlert(data['data']);
                        break;
     ...
     * 换箱列表
     */
    clickTransferBoxListChange(value) {
        switch (value) {
            case 1:
...
Tab1Page.propDecorators = {
    boxInfo: [{ type: _angular_core__WEBPACK_IMPORTED_MODULE_1__["ViewChild"], args: ['boxInfo', { static: false },] }]
};
